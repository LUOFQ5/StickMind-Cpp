...
public:
    ...
    //help function: if #define rec, else comment it
    int enqueueRec(DataPoint& elem, int index);
    ...
    DataPoint dequeue();
    //help function: if #define rec, else comment it
    int dequeueRec(DataPoint& front, int target);
...
private:
    ...
    int parenentIndex;  // the index of upper-layer element for a child element
    int leftChildIndex;  // the lower-left element for a father elemtnt
    int rightChildIndex;  // the lower-right element for a father element
    ...
    // helper function: enlarge _numAllocated while numFilled equals it;
    // double the value of _numAllocated;
    void enlarge();
    // helper function: get the index of the prioritized child from the two child element;
    // case1-doesnot have a child, return NONE; case2-only has the left child, return index of left child;
    // case3-return the index of the child whose prority is lower;
    int getSmallerChildIndex(int parent) const;
