...
/*
 * 错误版本--一个字符的差别：在递归中include模式时使用了+=
 * 分析：在每一个include/exclude的分支中，先调用include时会将该元素加入到sumSoFar，后调用exclude时的sumSoFar实际也加入了该元素；
 * 因此，导致的后果是难以形成exclude模式，实际上该分支exclude的结果与include结果相同。
 */
...
//触发错误的最小输入：Vector只有一个不为0的元素，如 {1}；
//预期结果为1（exclude模式，空集），buggy代码结果为0（此时exclude模式下，sum=1）
STUDENT_TEST("Buggy test: countZeroSumSubsets against buggyCount") {
    Vector<int> nums = {1};
